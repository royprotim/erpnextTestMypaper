...
var data=[],prev_row=null;this.data.forEach(function(d){if(prev_row&&d.name==prev_row.name){var new_row={};columns.forEach(function(c){if(!c.docfield||c.docfield.parent!==me.doctype){var val=d[c.field];if(c.docfield&&c.docfield.parent!==me.doctype){new_row[c.docfield.parent+":name"]=d[c.docfield.parent+":name"];}}else{var val='';}
...
this.chart_area.toggle(false);this.report_ajax=frappe.call({method:"frappe.desk.query_report.run",type:"GET",args:{"report_name":me.report_name,filters:filters},callback:function(r){me.report_ajax=undefined;me.make_results(r.message);}});return this.report_ajax;},trigger_refresh:function(){var me=this;var filters=me.get_values();var missing=false;$.each(me.filters,function(k,_f){if(_f.df.reqd&&!filters[_f.df.fieldname]){missing=true;return;}});if(!missing){me.refresh();}},get_values:function(raise){var filters={};var mandatory_fields=[];$.each(this.filters||[],function(i,f){var v=f.get_parsed_value();if(f.df.hidden)v=f.value;if(v==='%')v=null;if(f.df.reqd&&!v)mandatory_fields.push(f.df.label);if(v)filters[f.df.fieldname]=v;})
...
this.page.add_menu_item(__("Print"),function(){frappe.ui.get_print_settings(false,function(print_settings){frappe.render_grid({grid:me.grid,title:me.page.title,print_settings:print_settings});});},true);this.filter_inputs.range&&this.filter_inputs.range.on("change",function(){me.refresh();});if(this.setup_chart_check)
...
function handleKeyDown(e,args){var ranges;if(!_grid.getEditorLock().isActive()){if(e.which==frappe.ui.keyCode.ESCAPE){if(_copiedRanges){e.preventDefault();clearCopySelection();_self.onCopyCancelled.notify({ranges:_copiedRanges});_copiedRanges=null;}}
